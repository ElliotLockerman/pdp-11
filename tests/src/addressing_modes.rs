...
fn relative_label_read() {
    ...
    assert_eq!(emu.reg_read_word(Reg::R0), 0o012);
    assert_eq!(emu.reg_read_word(Reg::PC), DATA_START + offset + prog.text.len() as u16);
}
...
fn relative_label_write() {
    ...
    assert_eq!(emu.reg_read_word(Reg::PC), DATA_START + prog.text.len() as u16 + offset);
}
...
fn immediate_label_read() {
    ...
    assert_eq!(emu.reg_read_word(Reg::PC), DATA_START + prog.text.len() as u16 - WORD_SIZE);
}
